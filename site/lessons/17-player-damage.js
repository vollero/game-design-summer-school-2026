// STEP 17
// Anche il giocatore puo' essere colpito.
// Dopo un colpo resta invulnerabile per qualche frame.
...
function spawnEnemy() {
  ...
  let enemy = { x: 0, y: 0, raggio: 22, velocita: 1.9 };
...
function checkPlayerEnemyCollisions() {
  ...
  for (let i = enemies.length - 1; i >= 0; i--) {
    ...
    if (dist(player.x, player.y, enemy.x, enemy.y) < player.raggio + enemy.raggio) {
      ...
      enemies.splice(i, 1);
      log("Colpito! vite rimaste: " + player.lives);
    }
  }
}
