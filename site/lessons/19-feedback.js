// STEP 19
// Il feedback visivo rende il gioco piu' chiaro.
// Ogni colpo crea piccole particelle colorate.
...
function spawnEnemy() {
  ...
  let enemy = { x: 0, y: 0, raggio: 22, velocita: 1.8 + random(0.9) };
  ...
  for (let b = bullets.length - 1; b >= 0; b--) {
    for (let e = enemies.length - 1; e >= 0; e--) {
      if (dist(bullets[b].x, bullets[b].y, enemies[e].x, enemies[e].y) < bullets[b].raggio + enemies[e].raggio) {
        burst(enemies[e].x, enemies[e].y, "#facc15");
        bullets.splice(b, 1);
        enemies.splice(e, 1);
        score += 10;
        break;
...
function checkPlayerEnemyCollisions() {
  ...
  for (let i = enemies.length - 1; i >= 0; i--) {
    if (dist(player.x, player.y, enemies[i].x, enemies[i].y) < player.raggio + enemies[i].raggio) {
      burst(player.x, player.y, "#93c5fd");
      player.lives -= 1;
