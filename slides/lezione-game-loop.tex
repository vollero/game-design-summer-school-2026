...
\begin{frame}[fragile]{Input: il giocatore entra nel gioco}
  \begin{columns}[T]
    \begin{column}{0.6\textwidth}
      \begin{lstlisting}[style=jsstyle,basicstyle=\ttfamily\scriptsize]
if (keyIsDown(LEFT_ARROW) || keyIsDown("a")) {
  player.angle -= player.rotazione;
}
...
if (keyIsDown(UP_ARROW) || keyIsDown("w")) {
  player.x += Math.cos(player.angle) * velocita;
  player.y += Math.sin(player.angle) * velocita;
}
      \end{lstlisting}
    \end{column}
    \begin{column}{0.34\textwidth}
      \centering
      \begin{tikzpicture}
        \node[draw, rounded corners=5pt, minimum width=0.9cm, minimum height=0.8cm, fill=white] at (0,0.8) {$\uparrow$};
        \node[draw, rounded corners=5pt, minimum width=0.9cm, minimum height=0.8cm, fill=white] at (0,-0.3) {$\downarrow$};
        \node[draw, rounded corners=5pt, minimum width=0.9cm, minimum height=0.8cm, fill=white] at (-1.1,-0.3) {$\leftarrow$};
        \node[draw, rounded corners=5pt, minimum width=0.9cm, minimum height=0.8cm, fill=white] at (1.1,-0.3) {$\rightarrow$};
        \draw[Blue, thick, -{Latex[length=3mm]}] (-0.8,-1.05) arc[start angle=210,end angle=320,radius=0.9];
        \draw[Green, thick, -{Latex[length=3mm]}] (0,-1.9) -- (0,-1.15);
        \player{0.55}{-1.5}
      \end{tikzpicture}
    \end{column}
  \end{columns}
    ...
    \begin{column}{0.48\textwidth}
      \begin{block}{Cosa c'e' dentro}
        \begin{itemize}
          \item game loop;
          \item arena 1440x960;
          \item controllo a rotazione;
          \item sparo automatico singolo o triplo;
          \item bonus arma e riparazione;
          \item due tipi di nemici;
          \item collisioni;
          \item punteggio, vite e restart.
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.48\textwidth}
      ...
      \begin{tikzpicture}[scale=0.82]
        ...
        \shot{5.15}{2.75}
        \shot{5.05}{2.25}
        \draw[fill=Blue!35, draw=Blue, line width=1pt] (2.2,1.1) circle (0.22);
        \node[white, anchor=west] at (2.55,1.1) {bonus};
